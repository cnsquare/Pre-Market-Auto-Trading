...
def auto_trade():
    print("Starting script... " + datetime.datetime.now().__str__() + "\n")
...
def data_refinement(gainers, prices, volumes):
    ...
    for stock in gainers:
        if float(volumes[n]) > 80000:  # At least this volume
            cost = float(robinhood.bid_price(stock))
            if cost < 80:  # Cutoff price
...
def sellme():
    if did_you_buy:
        ...
        print("You need to buy before you can sell")
...
def sell(stock, num_stock):
    ...
    print("Total amount earned :" + float(total_cost + remaining).__str__())
